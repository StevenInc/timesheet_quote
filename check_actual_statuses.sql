-- Check what status values are actually available in the database
-- This will help us understand what statuses we can use

-- Check all enum types that might contain quote statuses
SELECT
  t.typname as enum_name,
  e.enumlabel as enum_value
FROM pg_type t
JOIN pg_enum e ON t.oid = e.enumtypid
WHERE t.typname LIKE '%quote%' OR t.typname LIKE '%status%'
...
WHERE table_name IN ('quotes', 'quote_revisions', 'quote_status_history')
